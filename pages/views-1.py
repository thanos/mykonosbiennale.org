...
class PageView(BuildableDetailView, View):
    build_path = ''
    queryset = models.Page.objects.filter(visible=True)
    
    def pages(self):
      return models.Page.objects.filter(visible=True)
...
#         return render_to_response(page.template, {'current_page':page, 'pages': models.Page.objects.filter(visible=True)})
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({'current_page':page, 'pages': models.Page.objects.filter(visible=True)})
        return context
