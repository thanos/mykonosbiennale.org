...
from imagelabs.models import ProcessedImage
from imagelabs import processor
        ...
        for film in Film.objects.exclude(poster=None):
            ...
            if poster:
                try:
                    ...
                    if  sz < 89478485:
                           print film.poster.url
                           print processor.process(film.poster.url,  width=150, height=300)
                    else:
                        print '>>>>>'
            ...
            if image.image:
                try:
                    ...
                    if  sz < 89478485:
                        ...
                        try:
                            processed_image = ProcessedImage.objects.get(source_image=image.image.url)
                            print 'fetched', processed_image
                        except ProcessedImage.DoesNotExist:
                            print   processor.process(image.image.url,  width=300, height=200)
                    else:
                        print >> sys.stderr, '>>>>>'
